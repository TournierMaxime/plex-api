...
class UserControllers {
    ...
    async getUserHistory(req, res) {
        ...
        const parsedAccountId = typeof accountId === "string" && !isNaN(Number(accountId))
            ? Number(accountId)
            : undefined;
        const parsedLibrarySectionID = typeof librarySectionID === "string" && !isNaN(Number(librarySectionID))
        ...
        const [sessionsHistory, devices] = await Promise.all([
            this.plexAPI.sessions.getSessionHistory(parsedSort, parsedAccountId, parsedFilter, parsedLibrarySectionID),
            this.plexAPI.server.getDevices(),
        ]);
        const devicesArray = devices.object?.mediaContainer?.device ?? [];
        const enrichedMetadata = sessionsHistory.object?.mediaContainer?.metadata
            ?.filter((a) => a.accountID === parsedAccountId)
            .map((item) => {
            const deviceMatch = devicesArray.find((d) => String(d.id) === String(item.deviceID));
            return {
                ...item,
